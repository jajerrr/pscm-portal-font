<template>
  ...
  <!-- ใช้ SwiperSlide เพื่อแสดง selectedItems -->
  <SwiperSlide
...
<div v-else>
  ...
  <!-- <div class="flex absolute bottom-3 left-10 z-[20] hide">
      <button v-for="(button,index) in buttonname" :key="index" 
        ...
        @click="selectCardsById(1)">
        <img :src="getContentImageSrc(button.icons)" />
        <div>{{ button.name }}</div>
      </button>
    </div> -->
    <button @click="selectCardsById(1)">Show Products (ID: 1)</button>
  </div>
</template>
...
// ฟังก์ชันเลือกข้อมูลตาม id
const selectCardsById = (id) => {
  const selectedCard = cards.valu.find((card) => card.id === id);
  selectedItems.value = selectedCard ? selectedCard.items : [];
};
