<template>
  <div
    class="w-full h-auto relative text-[#2B2B2B] SF-TH overflow-hidden scrollContainer cursor-default"
  >
    <ELearningNav2 />
<NuxtLayout name="blog">      
        <div class="relative">
          <div
            class="flex justify-between items-center pt-[2rem] ml-[5vw] mr-[5vw]"
            :class="{ 'max-h-[200px] overflow-y-auto': expandedIndex !== null }"
          >
            <h1 class="text-nm SF-TH-Semi">
              กระดานสนทนาศิษย์เก่าและศิษย์ปัจจุบัน
            </h1>
            <NuxtLink to="/blog">
              <button
                type="button"
                class="rounded-full flex items-center space-x-[2rem] px-3 py-2 hover:bg-[#ffe1c5] transition ease-in-out"
              ...
              >
                ...
                <span class="text-sm text-[#FF7A00]"
                  >กระทู้อาจารย์ในวิทยาลัยฯ</span
                >
              </button></NuxtLink
            >
          </div>
          ...
          <div class="flex mt-[1%] space-x-7 ml-[5vw]">
            <button
              type="button"
              class="rounded-full flex items-center space-x-[1rem] px-6 py-2 hover:bg-[#ffe1c5] transition ease-in-out"
              style="border-width: 2px; border-color: #ff7a00"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="h-[80vh] w-full scrollable-content ">
          <div
            v-for="(blog, index) in contentBlog"
            :key="index"
            @click="toggle(index)"
            :class="[
          ...
          >
              ...
              <div class="flex justify-between">
                <div class="flex mt-4">
                  <div class="flex items-center">
                    <img
                      :src="getContentImageSrc(blog.user)"
                      class="w-[1.5vw] h-[1.5vw] rounded-full"
                    />
                    <h class="ml-2 SF-TH text-sm text-[#FF7A00]">{{
                      blog.name
                    }}</h>
                  </div>
                  <div class="flex items-center">
                ...
                <div class="flex items-center space-x-4 mt-4">
                  ...
                  <span class="text-[gray-500]">
                    {{ blog.comments.length }} ความคิดเห็น</span
                  >
                </div>
              </div>
            </div>
            ...
            <div v-if="expandedIndex === index" class="mt-4 border-t pt-4">
              ...
              <div class="scrollable-text">
                <!-- แสดงคอมเมนต์ -->
                <div
                  v-for="(comment, cIndex) in blog.comments"
                  :key="cIndex"
                  class="flex mt-2 pl-[3%]"
                >
                  <img
              ...
              <div class="mt-4" @click.stop>
                ...
                <div class="flex justify-end">
                  <button
                    @click.stop="submitComment(index)"
...
const submitComment = (index) => {
  ...
  contentBlog.value[index].comment =
    contentBlog.value[index].comments.length.toString(); // อัปเดตจำนวนคอมเมนต์
