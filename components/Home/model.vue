...
scene.background = null  // หรือใช้ 'transparent' ขึ้นอยู่กับความต้องการ
  ...
  controls.minDistance = 2.8  // ค่าต่ำสุดของระยะทางกล้องจากโมเดล
  controls.maxDistance = 2.8  // ค่าสูงสุดของระยะทางกล้องจากโมเดล
  ...
  directionalLight.position.set(5, 5, 5)  // ตั้งตำแหน่งของแสงทิศทาง
  scene.add(directionalLight)  // เพิ่มแสงทิศทางลงในฉาก

  // โหลดโมเดล GLB
  const loader = new GLTFLoader()  // สร้างตัวโหลดเพื่อโหลดโมเดล GLTF/GLB
  loader.load(
    'http://27.254.62.100:30021/e-museum-prod/models/BP-1.glb',  // URL ของโมเดลที่ต้องการโหลด
    (gltf) => {
